...
--
-- Name: export_images(); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION export_images() RETURNS void
    LANGUAGE plpgsql
    AS $$
    BEGIN
        COPY
        (
            ...
            CONCAT( 'https://images.ala.org.au/image/proxyImageThumbnailLarge?imageId=', image_identifier) AS identifier,
            ...
            CONCAT('https://images.ala.org.au/image/', image_identifier) AS "references",
            regexp_replace(title, E'[\\n\\r]+', ' ', 'g' ) as title,
...
--
-- Name: export_index(); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION export_index() RETURNS void
    LANGUAGE plpgsql
    AS $_$
BEGIN
    COPY
