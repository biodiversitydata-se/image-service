...
class WebServiceController {
    ...
    @Operation(
            method = "POST",
            summary = "Schedule tile generation",
            description = "Schedule tile generation. Required scopes: 'image-service/write'.",
            parameters = [
    ...
    @Path("/ws/scheduleTileGenerationWS/{id}")
    @Consumes("application/json")
    @Produces("application/json")
    @RequireApiKey(scopes = ['image-service/write'])
    ...
    def scheduleTileGeneration() {
        ...
        if (params.id && !imageInstance) {
            ...
            if (imageInstance) {
                ...
                results.message = "Image tile generation scheduled for image ${imageInstance.id}"
            } else {
                def count = forEachImageId { imageId ->
                    imageService.scheduleTileGeneration(imageId, userId)
                }
                results.message = "Image tile generation scheduled for ${count} images."
            }
